...
static void syscall_handle_exec(interrupt_state * state) {
    ...
    user_path = (const char *)state->ebx;
    if (user_path == 0 || !is_user_vaddr((unsigned int)user_path)) {
        serial_printf("[syscall] invalid exec path ptr=0x%x\n", state->ebx);
        state->eax = SYSCALL_ERROR;
        return;
    }

    max_len = USER_VIRT_END - (unsigned int)user_path;
    if (max_len > SYSCALL_EXEC_PATH_MAX) {
        max_len = SYSCALL_EXEC_PATH_MAX;
    }

    for (i = 0; i < max_len; i++) {
        path_buffer[i] = user_path[i];
        if (path_buffer[i] == 0) {
            break;
        }
    }

    if (i == max_len && path_buffer[max_len - 1] != 0) {
        serial_printf("[syscall] exec path too long ptr=0x%x\n", state->ebx);
        state->eax = SYSCALL_ERROR;
        return;
    }
