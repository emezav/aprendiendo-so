...
/** @brief buffer del driver de teclado */
char keyboard_buf[BUFSIZ];

/** @brief posicion actual en el buffer del driver de teclado */
int keyboard_pos;

/** @brief Mapa del teclado sin teclas modificadoras */
char es_kb[KB_KEYS];

/** @brief Mapa del teclado con SHIFT pulsado */
char es_kb2[KB_KEYS];

/** @brief Mapa del teclado con ALT GR (CTRL SHIFT) pulsado */
char es_kb3[KB_KEYS];

/** @brief Almacena el estado de las teclas  modificadoras*/
...
void keyboard_handler(interrupt_state * state) {
    ...
    if (!(status & KB_HAS_OUTPUT)) {
        //No hay datos en el registro de salida?
        return;
    }
