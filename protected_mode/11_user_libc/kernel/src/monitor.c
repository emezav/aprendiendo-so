...
static void monitor_copy_text(char * dst, int dst_size, const char * src);
...
int monitor_register_command(const char * name, monitor_callback callback,
    ...
    monitor_copy_text(monitor_commands[monitor_command_count].name,
            MONITOR_MAX_COMMAND_NAME, name);
    monitor_commands[monitor_command_count].callback = callback;

    if (help != 0) {
        monitor_copy_text(monitor_commands[monitor_command_count].help,
                MONITOR_MAX_HELP_TEXT, help);
    } else {
