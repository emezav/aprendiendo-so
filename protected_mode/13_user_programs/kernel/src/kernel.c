...
static void monitor_command_shell(char * args);
...
void cmain(){
    ...
    /* Abrir la puerta minima de llamadas al sistema. */
    setup_syscalls();
    serial_printf("[serial] syscall interrupt initialized\n");
    ...
    /* Preparar el lector minimo de ext2 sobre la base ya validada de disco. */
    setup_ext2();
    serial_printf("[serial] ext2 initialized\n");

    /* Preparar el cargador minimo de programas ELF desde ext2. */
    setup_user_exec();
    serial_printf("[serial] user exec initialized\n");
    ...
    register_ext2_monitor_commands();
    register_usermode_monitor_commands();
    register_syscall_monitor_commands();
    register_user_exec_monitor_commands();
...
void kernel_enter_monitor_loop(void) {
    ...
    /* Si el monitor se reingresa desde una interrupcion, el procesador puede
     * llegar con IF=0. Volver a habilitar interrupciones aqui permite que el
     * timer y el teclado sigan despertando el bucle del monitor. */
    inline_assembly("sti");
...
static void register_kernel_monitor_commands(void) {
    ...
    monitor_register_command("clear", monitor_command_clear,
            "clear the screen");
    monitor_register_command("shell", monitor_command_shell,
            "run /boot/shell.elf in mode de usuario");
...
static void monitor_command_shell(char * args) {
    (void)args;
    if (!userexec_run_path("/boot/shell.elf")) {
        klog_printf("shell: could not launch /boot/shell.elf\n");
    }
}
