...
void interrupt_dispatcher() {
    ...
    /* Si en el momento de ocurrir la interrupcion existe una tarea actual,
	 * actualizar su atributo current_ss y current_esp con los valores
	 * actualizados por la rutina de servicio de interrupcion correspondiente.
	 */
	if (current_task != NULL_TASK) {
        current_task->current_esp = current_esp;
	}
