...
#define SYSCALL_TEST_CODE_VADDR (USER_VIRT_START + 0x00012000)
/** @brief Pagina virtual de la pila del payload de prueba de syscalls. */
#define SYSCALL_TEST_STACK_VADDR (SYSCALL_TEST_CODE_VADDR + PAGE_SIZE)
...
static void syscall_handle_exit(interrupt_state * state) {
    ...
    klog_printf("syscall: int 0x80 path verified; kernel halted intentionally\n");
    serial_printf("[syscall] halting after successful int 0x80 test\n");
    for (;;) {
        inline_assembly("cli");
        inline_assembly("hlt");
    }
