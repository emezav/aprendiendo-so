...
/** @brief Selector reservado para el segmento de codigo de userland. */
unsigned short user_code_selector = 0;

/** @brief Selector reservado para el segmento de datos de userland. */
unsigned short user_data_selector = 0;
...
/**
 * @brief Reserva y configura los segmentos minimos de codigo y datos para userland.
 */
void setup_user_segments(void) {
    if (user_code_selector == 0) {
        user_code_selector = allocate_gdt_selector();
    }

    if (user_data_selector == 0) {
        user_data_selector = allocate_gdt_selector();
    }
    ...
    setup_gdt_descriptor(user_code_selector, 0x00000000, 0xFFFFFFFF,
            CODE_SEGMENT, RING3_DPL, 1, 1);
    setup_gdt_descriptor(user_data_selector, 0x00000000, 0xFFFFFFFF,
            DATA_SEGMENT, RING3_DPL, 1, 1);
}
