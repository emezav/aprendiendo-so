...
/**
 * @brief Mapea una pagina usando los flags indicados en la entrada de pagina.
 * @param vaddr Direccion virtual a mapear.
 * @param addr Direccion fisica del marco de pagina.
 * @param page_flags Flags de presencia y privilegio a almacenar.
 * @return 1 si el mapeo fue exitoso, 0 en caso contrario.
 */
static int map_page_with_flags(unsigned int vaddr, unsigned int addr,
        unsigned int page_flags);

static unsigned int get_directory_flags(unsigned int page_flags);
...
unsigned int create_new_page_table(int pd_entry,
     ...
     * KERNEL_PAGETABLES_VADDR */
    kernel_pd[pd_entry] = frame_addr | directory_flags;
...
static int map_page_with_flags(unsigned int vaddr, unsigned int addr,
    ...
    directory_flags = get_directory_flags(page_flags);
    ...
    } else if (directory_flags & PG_USER) {
        kernel_pd[pd_entry] |= PG_USER;
    }
     ...
     * correspondiente. */
    pt[pt_entry] = addr | page_flags;

    return 1;
}

static unsigned int get_directory_flags(unsigned int page_flags) {
    if (page_flags & PG_USER) {
        return PG_USER_PRESENT;
    }

    return PG_KERNEL_PRESENT;
}

/**
 * @brief Permite mapear una pagina del kernel a un marco de pagina.
 * @param vaddr Direccion virtual de la pagina del kernel a mapear.
 * @param addr Direccion fisica del marco de pagina.
 * @return 1 si el mapeo fue exitoso, 0 en caso contrario.
 */
int map_page(unsigned int vaddr, unsigned int addr) {
    return map_page_with_flags(vaddr, addr, PG_KERNEL_PRESENT);
}

/**
 * @brief Mapea una pagina de userland con permisos de usuario.
 * @param vaddr Direccion virtual de userland a mapear.
 * @param addr Direccion fisica del marco de pagina.
 * @return 1 si el mapeo fue exitoso, 0 en caso contrario.
 */
int map_user_page(unsigned int vaddr, unsigned int addr) {
    if (!is_user_vaddr(vaddr)) {
        return 0;
    }

    return map_page_with_flags(vaddr, addr, PG_USER_PRESENT);
}

/**
 * @brief Verifica si una direccion virtual esta dentro del rango provisional de userland.
 * @param vaddr Direccion virtual a validar.
 * @return 1 si la direccion pertenece al rango provisional de userland, 0 en caso contrario.
 */
int is_user_vaddr(unsigned int vaddr) {
    unsigned int page;

    page = ROUND_DOWN_TO_PAGE(vaddr);
    return page >= USER_VIRT_START && page < USER_VIRT_END;
}
...
void page_fault_handler(interrupt_state * state) {
    ...
    serial_printf("[fault] page fault vaddr=0x%x page=0x%x eip=0x%x cs=0x%x err=0x%x\n",
            vaddr, page, state->old_eip, state->old_cs, state->error_code);
    for (;;);
}
