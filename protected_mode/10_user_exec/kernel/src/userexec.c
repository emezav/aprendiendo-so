...
#define USEREXEC_ELF_BASE (USER_VIRT_START + 0x00010000)
#define USEREXEC_STACK_BASE (USEREXEC_ELF_BASE + 0x00010000)
...
static void userexec_command_run(char * args) {
    ...
    userexec_entry_point = USEREXEC_ELF_BASE + header->entry;
...
static int userexec_map_segment(const unsigned char * file_data,
    ...
    start_vaddr = ROUND_DOWN_TO_PAGE(USEREXEC_ELF_BASE
            + program_header->virtual_address);
    end_vaddr = ROUND_UP_TO_PAGE(USEREXEC_ELF_BASE
    ...
    for (offset = 0; offset < program_header->file_size; offset++) {
        ...
        dst_vaddr = USEREXEC_ELF_BASE + program_header->virtual_address + offset;
    ...
    serial_printf("[userexec] mapped load segment vaddr=0x%x filesz=%u memsz=%u\n",
            USEREXEC_ELF_BASE + program_header->virtual_address,
